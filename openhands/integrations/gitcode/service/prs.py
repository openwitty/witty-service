...
class GitCodePRsMixin(GitCodeMixinBase):
    """Pull request helpers for GitCode."""
    ...
    async def create_pull_request(self, data: dict[str, Any] | None = None) -> dict:
        ...
        if not isinstance(response, dict):
            raise UnknownException('Unexpected response creating GitCode pull request')
    ...
    async def request_reviewers(
        ...
        url = self._build_repo_api_url(
            owner, repo, 'pulls', str(pr_number), 'assignees'
        )
        ...
        try:
            ...
            logger.warning(
                'Failed to request GitCode reviewers %s for %s/%s PR #%s: %s',
                reviewers,
                owner,
                repo,
                pr_number,
