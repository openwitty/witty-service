...
class GitCodeMixinBase(BaseGitService, HTTPClient):
    """Common functionality shared by GitCode (AtomGit) service mixins."""

    DEFAULT_BASE_URL = 'https://api.gitcode.com/api/v5'
    DEFAULT_DOMAIN = 'gitcode.com'
    DEFAULT_WEB_BASE_URL = 'https://gitcode.com'
    ...
    def __init__(
        ...
        env_base_url = os.environ.get('GITCODE_BASE_URL')
        self.BASE_URL = self._resolve_base_url(base_url, base_domain, env_base_url)
        self.base_url = self.BASE_URL  # Backwards compatibility

        parsed = urlparse(self.BASE_URL)
        self.base_domain = base_domain or parsed.netloc or self.DEFAULT_DOMAIN

        # Web base is used for UI-friendly URLs where needed.
        web_domain = self.DEFAULT_DOMAIN
        if base_domain and 'gitcode.com' in base_domain:
            web_domain = 'gitcode.com'
        self.web_base_url = f'https://{web_domain}'.rstrip('/')

    @property
    def provider(self) -> str:
        return ProviderType.GITCODE.value
    ...
    def _resolve_base_url(
        ...
        for candidate in (explicit_base_url, env_base_url, base_domain):
            if not candidate:
                continue
            ...
            # If the candidate already points at an API path, use it directly.
            if '/api/' in url:
                return url

            # GitCode/AtomGit uses a dedicated API host.
            if 'gitcode.com' in url or 'atomgit.com' in url:
    ...
    async def get_user(self) -> User:  # type: ignore[override]
        ...
        return User(
            ...
            name=response.get('name') or response.get('full_name'),
            email=response.get('email'),
            company=response.get('company') or response.get('organization'),
        )
    ...
    def _parse_repository(
        ...
        owner_login = (
            owner.get('login') or owner.get('username') or owner.get('name') or ''
        )
        full_name = (
            repo.get('full_name')
            or repo.get('path_with_namespace')
            or repo.get('full_path')
        )
        if not full_name:
        ...
        if is_private is None:
            is_private = not bool(repo.get('public', True))

        return Repository(
            id=str(repo.get('id', '')),
            full_name=str(full_name or ''),
            stargazers_count=repo.get('stargazers_count')
            or repo.get('stars_count')
            or repo.get('watchers_count'),
            git_provider=ProviderType.GITCODE,
            is_public=not bool(is_private),
            link_header=link_header,
            pushed_at=repo.get('pushed_at') or repo.get('updated_at'),
            owner_type=owner_type,
            main_branch=repo.get('default_branch') or repo.get('default_branch_name'),
        )
