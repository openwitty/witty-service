...
| 环境变量 | 说明 | 默认值 |
|----------|------|--------|
| `WITTY_INSIGHT_ENABLED` | 是否启用 witty-insight 集成 | `false` |
...
默认推荐同机部署并通过 `127.0.0.1` 直连 `witty-insight`。修改上述环境变量后，
需要重启 `witty-service` 进程才能生效。

同机部署示例：

```bash
export WITTY_INSIGHT_ENABLED=true
